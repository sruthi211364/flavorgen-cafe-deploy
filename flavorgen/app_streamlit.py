...
# ── Path setup (works locally AND on Hugging Face) ──────────
...
try:
    ...
    import streamlit as st
    st.error(f"❌ data_loader import failed: {_e}")
    raise
...
try:
    ...
    import streamlit as st
    st.error(f"❌ semantic_model import failed: {_e}")
    raise
...
try:
    ...
    import streamlit as st
    st.error(f"❌ fusion_model import failed: {_e}")
    raise

try:
    from flavorgen.mongo_client import (
        get_db,
        db_register_user,
        db_login_user,
        db_save_interests,
        db_save_favourites,
        db_load_favourites,
        db_save_generated_drink,
        db_load_generated_drinks,
        db_delete_generated_drink,
        db_get_user,
    )
except Exception as _e:
    import streamlit as st
    st.error(f"❌ mongo_client import failed: {_e}")
    raise
...
def login_user(username: str, pw: str) -> bool:
    user = db_login_user(username, pw)
    if user:
        st.session_state.current_user   = username
        st.session_state.favorites      = set(int(x) for x in user.get("favourites", []))
        st.session_state.user_interests = user.get("interests", [])
        st.session_state.user_display   = user.get("display_name", username)
        return True
    return False


def register_user(username: str, pw: str, display: str) -> Tuple[bool, str]:
    return db_register_user(username, pw, display)
...
def save_user_profile() -> None:
    ...
    if not u or u == "__guest__":
        return
    db_save_favourites(u, sorted(list(st.session_state.get("favorites", set()))))
    db_save_interests(u, st.session_state.get("user_interests", []))
...
def ensure_session() -> None:
    defaults = {
        ...
        "ai_text_meta":    {},
        "fusion_last":     None,
        "active_page":     "auth",   # FIX 1: was "home" — start on auth until logged in
        "_show_login":     True,
    }
...
def render_recipe_card(out: dict) -> None:
    ...
    meta_pills = "".join([f"<span class='recipe-meta-pill'>🌿 {t}</span>" for t in top3])
...
def render_home_navigation() -> None:
    ...
    _nav_tile(c7,"✨","Today's Special","Hand-picked daily highlight just for you.",
              ...
              card_style="background:linear-gradient(150deg,rgba(255,249,225,.98),rgba(255,238,190,.92));border-color:rgba(212,137,26,.30);",
              title_color="#7a3e00",
...
""", unsafe_allow_html=True)
            ...
            with st.expander("🎨 My Taste Interests", expanded=False):
                ...
                selected = st.multiselect("Flavours I love", all_dims, default=current, key="sidebar_interests")
                if selected != current:
...
def page_mystery() -> None:
    render_hero("Mystery Roller 🎲","Feeling adventurous? Let the café choose for you.",
...
<div class="roll-result">
  <div style="font-size:11px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:var(--caramel);margin-bottom:4px;">You rolled</div>
...
""", unsafe_allow_html=True)
            ...
            with meta_col:
                ...
                if st.button("❤️ Remove favourite" if is_fav else "🤍 Add to favourites", key="mystery_fav_btn"):
                    toggle_favorite(int(picked_id))
                ings = top_ingredients_for_drink(r, k=6)
                if ings: st.markdown("**Key ingredients:** " + ", ".join(ings))
            with chart_col:
...
# ════════════════════════════════════════════════════════════
def page_menu() -> None:
    render_hero("Browse Menu ☕","Filter, search, and discover drinks from the full catalogue.",
...
""", unsafe_allow_html=True)
    ...
    with st.expander("🔍 Drink detail view", expanded=False):
        if view_df.empty: st.info("No drinks available."); return
...
def page_favorites() -> None:
    render_hero("My Favourites ❤️","Your saved drinks, personal flavour profile, and taste summary.",
    ...
    if not fav_ids:
        st.info("No favourites yet — browse or roll a drink and hit ❤️"); return
...
def page_fusion_lab() -> None:
    render_hero("Fusion Lab 🧪","Blend two drinks into a brand-new creation — get an interactive recipe card.",
    ...
    with cs:
        if st.button("💾 Save this fusion", key="btn_fusion_save"):
            if not st.session_state.get("fusion_last"):
                ...
                try:
                    uname = st.session_state.get("current_user", "__guest__")
                    db_save_generated_drink(uname, st.session_state.fusion_last)
...
def page_generated_drinks() -> None:
    render_hero("Generated Drinks 📂","Your saved fusion creations as interactive recipe cards.",
                ["🗂️ Saved","📋 Recipe view","⬇️ Download"])
    uname = st.session_state.get("current_user", "__guest__")
    items = db_load_generated_drinks(uname, limit=50) if uname != "__guest__" else []
    if not items:
        st.info("No generated drinks yet. Create one in Fusion Lab."); return
    st.download_button("⬇️ Download all (JSON)",
                       json.dumps(items, indent=2, default=str).encode(),
    ...
    for item in items:
        ...
        with col_del:
            ...
            if st.button("🗑️ Delete", key=del_key):
                try:
                    uname2 = st.session_state.get("current_user", "__guest__")
                    db_delete_generated_drink(uname2, item.get("created_at", 0))
...
# ════════════════════════════════════════════════════════════
# SIDEBAR + ROUTER  ← FIX 2: unified guard, no ambiguous elif
# ════════════════════════════════════════════════════════════
render_sidebar()
...
# Always gate on login first — no matter what active_page says
if user is None or ap == "auth":
    page_auth()
elif ap == "home":
    render_hero("Flavorgen Café Lab ☕","A cozy café experience to browse, recommend, and generate drinks.",
